CREATE TABLE public.discovery_vmware_inventory_cluster (
	id varchar DEFAULT nextval('vmware_inventory_cluster_id_seq'::regclass) NOT NULL,
	...
	CONSTRAINT uq_cluster_hier UNIQUE (parent_component_moid, component_moid),
	CONSTRAINT vmware_inventory_cluster_pkey PRIMARY KEY (id)
);
...
create trigger update_cluster_last_observed before
update
    on
    public.discovery_vmware_inventory_cluster for each row execute function update_last_observed_column();